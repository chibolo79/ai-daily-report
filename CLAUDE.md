...
### `/image-organizer`
**트리거**: "이미지 정리", "명함 정리", "사진 정리", "이미지 올렸어", "명함 저장했어"  
...
**논의사항**:
- **기존 논의사항**: (이전 방문 내용 전체 통합, 날짜 생략) ← 이전 방문이 있는 경우만
- **금번 논의사항**: (이번 출장 내용) ← 이전 방문이 없으면 기존/금번 구분 없이 내용만 서술
**FOLLOW UP**: ...
```
...
| 규칙 | 내용 |
|------|------|
| **영문 표기** | 모든 영문은 대문자. 담당자명·직함·품목·지명·인증명 모두 포함. (예: SWP-B, SUS304, GENERAL DIRECTOR, BAC NINH) |
| **WIRE 사이즈** | 소수점 둘째 자리까지 고정 표기. (예: 0.20MM, 0.45MM, 3.00MM) |
| **범위 표기** | `~` 대신 `-` 사용. 마크다운 취소선 방지. (예: 0.12-1.60MM, 14-15톤) |
| **어투** | 보고서 문체. `~입니다/있습니다/하였습니다` 금지 → `~임/있음/하였음`으로 통일 |
| **서술 방식** | 결재권자가 배경지식 없이 읽어도 이해할 수 있도록 서술형으로 풀어 작성. 키워드 나열 금지. 수치의 의미·배경·영향도를 함께 기재 |
| **논의사항 구조** | `기존 논의사항` = 이전 방문 전체 통합 (날짜·차수 생략), `금번 논의사항` = 이번 출장 내용 |
