...
- **영문 표기**: 모든 영문은 대문자. (예: SWP-B, SUS304, PIANO WIRE, HA NAM, GENERAL DIRECTOR)
- **WIRE 사이즈**: 소수점 둘째 자리까지 고정 표기. (예: 0.20MM, 0.45MM, 3.00MM)
- **서술 방식**: 결재권자(임원급)가 배경지식 없이 읽어도 이해할 수 있도록 서술형으로 풀어 작성. 단순 키워드 나열 금지. 각 항목마다 수치의 의미·배경·영향도를 함께 기재. 방문 이력은 방문별로 독립 단락으로 구분하여 당시 상황과 결과를 서술.
- **어투**: 보고서 문체 사용. `~입니다 / ~있습니다 / ~하였습니다` 형식 금지. `~임 / ~있음 / ~하였음` 형식으로 통일. (예: "공급 중임." / "가능성 있음." / "확인하였음.")
